...
def test_invalid_msg_id(clear):
    ...
    response = requests.post(config.url + 'message/unreact/v1', json={'token': token, 'message_id': 2, 'react_id': 1})
...
def test_invalid_react_id(clear):
    ...
    requests.post(config.url + 'message/react/v1', json={'token': token, 'message_id': message_id, 'react_id': 1})

    response = requests.post(config.url + 'message/unreact/v1', json={'token': token, 'message_id': message_id, 'react_id': -2})
    assert response.status_code == 400

# Input Error: the message does not contain a react with ID react_id from the authorised user
def test_does_not_contain_react(clear):
    token = create_valid_token()
    channel_id = create_valid_channel_id(token)
    message_id = send_message(token, channel_id)
...
def test_unreact_success(clear):
    token = create_valid_token()
    channel_id = create_valid_channel_id(token)
    message_id = send_message(token, channel_id)
    ...
    response = requests.post(config.url + 'message/unreact/v1', json={'token': token, 'message_id': message_id, 'react_id': 1})
