...
def pin_message(token, message_id):
    requests.post(config.url + 'message/pin/v1', json={'token': token, 'message_id': message_id})
    return
#------------------------------------------------
...
def test_invalid_msg_id(clear):
    ...
    pin_message(token, message_id)
    response = requests.post(config.url+'message/unpin/v1', json={'token': token, 'message_id': -1})
    assert response.status_code == 400

def test_message_not_in_user_channels(clear):
    token = create_valid_token()
    channel_id = create_valid_channel_id(token)
    message_id = send_message(token, channel_id)
    ...
    pin_message(token, message_id)
    response = requests.post(config.url+'message/unpin/v1', json={'token': token2, 'message_id': message_id})
    assert response.status_code == 400

def test_not_already_pinned(clear):
    token = create_valid_token()
    channel_id = create_valid_channel_id(token)
    message_id = send_message(token, channel_id)

    response = requests.post(config.url+'message/unpin/v1', json={'token': token, 'message_id': message_id})
    assert response.status_code == 400


def test_no_owner_permissions(clear):
    token = create_valid_token()
    channel_id = create_valid_channel_id(token)
    message_id = send_message(token, channel_id)
